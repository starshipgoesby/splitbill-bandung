...
import {
  ...
  Sparkles, Loader2, CreditCard, ChevronRight, WifiOff,
} from "lucide-react";
...
const sb = {
  async getTrip() {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/trip_data?id=eq.${TRIP_ID}&select=data`, {
...
function computeReceiptShares(items, charges, members) {
  ...
  members.forEach((m) => {
    if (base[m.id] > 0) {
      shares[m.id] = Math.round(base[m.id] + (subtotal > 0 ? adj * (base[m.id] / subtotal) : 0));
    }
  });
...
// ── Detail Modal ──────────────────────────────────────────────────────
function DetailModal({ expense, members, onClose }) {
  ...
  const dateStr = new Date(expense.at || Date.now()).toLocaleString("id-ID", {
          ...
          {expense.hasReceipt && (
            <div style={{ marginTop: 18 }}>
              ...
              <div style={{ marginTop: 8, background: "#fff", borderRadius: 14, padding: 8 }}>
                {photoState === "loading" && (
                  <div style={{ padding: "30px 0", textAlign: "center", color: "#a8987f" }}>
                    <Loader2 size={22} style={{ animation: "spin 1s linear infinite" }} />
                    <div style={{ fontSize: 13, marginTop: 8 }}>Memuat foto…</div>
                  </div>
                )}
                {photoState === "ok" && photo && (
                  <img src={photo} alt="Struk" style={{ width: "100%", borderRadius: 10, display: "block", maxHeight: 420, objectFit: "contain", background: "#f5f0e8" }} />
                )}
                {photoState === "missing" && (
                  <div style={{ padding: "26px 0", textAlign: "center", color: "#a8987f", fontSize: 13 }}>
                    Foto tidak tersedia.
                  </div>
                )}
              </div>
              ...
              {expense.items.map((it, idx) => (
                <div key={idx} style={styles.detailItem}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 6 }}>
                    {(it.who || []).map((id) => (
                      <span key={id} style={{ padding: "3px 9px", borderRadius: 14, fontSize: 11.5, fontWeight: 700, background: colorOf(id) + "22", color: colorOf(id) }}>
                        {nameOf(id)}
                      </span>
                    ))}
                  </div>
            ...
            {Object.entries(expense.shares || {}).map(([id, amt]) => (
              <div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #faf4ea" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {id === expense.paidBy && (
                    <span style={{ fontSize: 10.5, letterSpacing: 1, fontWeight: 700, color: "#a8432a", background: "#c75b3915", padding: "2px 7px", borderRadius: 10, textTransform: "uppercase" }}>nalangin</span>
                  )}
                </span>
...
function ScanModal({ members, onClose, onSave }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      setErrMsg("Gagal membaca struk. Coba foto yang lebih jelas."); setStep("error");
    }
  };
  ...
  const save = () => {
    ...
    onSave({ id: uid(), desc: merchant || "Struk", amount, paidBy, shares, at: Date.now(), scanned: true, items: items.map(({ name, price, who }) => ({ name, price, who })), charges: { ...charges }, hasReceipt: !!photoDataUrl }, photoDataUrl);
  };
      ...
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        {step === "review" && (
          <div style={{ maxHeight: "78vh", overflowY: "auto", margin: "-4px -4px 0", padding: 4 }}>
            {photoDataUrl && (
              <div style={{ marginBottom: 14, borderRadius: 12, overflow: "hidden", maxHeight: 160, background: "#f5f0e8" }}>
                <img src={photoDataUrl} alt="Struk" style={{ width: "100%", objectFit: "cover", display: "block", maxHeight: 160 }} />
              </div>
            )}
            <input value={merchant} onChange={(e) => setMerchant(e.target.value)}
              style={{ ...styles.input, fontFamily: "Fraunces, serif", fontSize: 20, fontWeight: 600, padding: "8px 12px", border: "1.5px dashed #e6d9c6" }} />
            ...
            {items.map((it) => (
              <div key={it.id} style={styles.itemCard}>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input value={it.name} onChange={(e) => setField(it.id, "name", e.target.value)}
                    style={{ ...styles.input, flex: 1, padding: "8px 10px", fontSize: 14 }} />
                  <input value={(it.price || 0).toLocaleString("id-ID")}
                    onChange={(e) => setField(it.id, "price", parseInt(e.target.value.replace(/\D/g, "") || "0", 10))}
                    inputMode="numeric" style={{ ...styles.input, width: 92, padding: "8px 10px", fontSize: 14, textAlign: "right" }} />
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                  {members.map((m) => {
                    ...
                    return (
                      <button key={m.id} onClick={() => toggleWho(it.id, m.id)}
                        style={{ ...styles.miniChip, ...(on ? { background: m.color, color: "#fff", borderColor: m.color } : {}) }}>
                        {m.name}
                      </button>
                    );
                  })}
            ...
            {[["Pajak (PB1)", "tax"], ["Service charge", "service"], ["Diskon", "discount"]].map(([label, key]) => (
              <div key={key} style={styles.chargeRow}>
                <span>{label}</span>
            ...
            <div style={styles.previewBox}>
              ...
              {members.filter((m) => preview.shares[m.id]).map((m) => (
                <div key={m.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 13.5, color: "#5a4d3e", padding: "2px 0" }}>
...
function ManualModal({ members, onClose, onSave }) {
  ...
  const save = () => {
    ...
    onSave({ id: uid(), desc: desc.trim(), amount: amt, paidBy, shares, at: Date.now() }, null);
  };
  return (
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <h3 style={styles.modalTitle}>Catat manual (bagi rata)</h3>
        ...
        <input value={amount ? parseInt(String(amount).replace(/\D/g, "") || "0", 10).toLocaleString("id-ID") : ""}
          onChange={(e) => setAmount(e.target.value.replace(/\D/g, ""))} placeholder="Jumlah (Rp)" inputMode="numeric"
          ...
          {members.map((m) => {
            ...
            return (
              <button key={m.id} onClick={() => toggle(m.id)}
                style={{ ...styles.splitChip, ...(on ? { background: m.color, color: "#fff", borderColor: m.color } : {}) }}>
...
export default function App() {
  ...
  useEffect(() => {
    sb.getTrip().then((data) => {
      if (data) { setMembers(data.members || []); setExpenses(data.expenses || []); }
      setLoading(false);
  ...
  const persist = async (m, e) => {
    setSaving(true);
    try { await sb.saveTrip({ members: m, expenses: e }); setOffline(false); }
  ...
  const addExpense = async (exp, photoDataUrl) => {
    ...
    if (photoDataUrl) {
      try { await sb.savePhoto(exp.id, photoDataUrl); }
    ...
    <div style={styles.page}>
      ...
      {offline && (
        <div style={{ background: "#c1121f", color: "#fff", padding: "10px 18px", display: "flex", gap: 8, alignItems: "center", fontSize: 13 }}>
          <WifiOff size={15} /> Tidak bisa terhubung ke server. Perubahan mungkin belum tersimpan.
        </div>
      )}
      ...
      <div style={styles.section}>
          ...
          {members.map((m) => (
            <div key={m.id} style={styles.memberCard}>
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                ...
                <button onClick={() => setEditAcct(editAcct === m.id ? null : m.id)} style={styles.acctBtn}>
                  <CreditCard size={13} /> {m.account ? "ubah" : "tujuan transfer"}
                </button>
                <button onClick={() => removeMember(m.id)} style={styles.iconDel}><Trash2 size={14} /></button>
              </div>
              {m.account && editAcct !== m.id && (
                <div style={{ fontSize: 12.5, color: "#8a7d6e", marginTop: 5, paddingLeft: 20 }}>{m.account}</div>
              )}
              {editAcct === m.id && (
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input value={newName} onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addMember()} placeholder="Tambah nama teman…" style={styles.input} />
      ...
      {tab === "expenses" && (
        <div style={styles.section}>
          ...
          {members.length > 0 && expenses.length === 0 && <div style={styles.empty}>Belum ada pengeluaran. Scan struk atau input manual.</div>}
          {expenses.map((e) => (
            <div key={e.id} className="row tap-card" style={styles.expenseTap} onClick={() => setOpen(e)}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={styles.expMeta}>Dibayar <b style={{ color: colorOf(e.paidBy) }}>{nameOf(e.paidBy)}</b> · {Object.keys(e.shares || {}).length} orang ikut</div>
              </div>
          ...
          {expenses.length > 0 && <div style={{ fontSize: 11.5, color: "#b3a596", textAlign: "center", marginTop: 12 }}>Tap untuk lihat detail & foto struk</div>}
        </div>
      )}
          ...
          {tx.length === 0 ? <div style={styles.empty}>Belum ada yang perlu ditransfer.</div> : tx.map((t, i) => (
            <div key={i} className="row" style={styles.settle}>
              ...
              <div style={{ fontSize: 12.5, color: "#8a7d6e", marginTop: 6 }}>
                {t.to.account
                  ? <>Transfer ke: <b style={{ color: "#5a4d3e" }}>{t.to.account}</b></>
                  : <span style={{ fontStyle: "italic" }}>Tambah no. rekening {t.to.name} di bagian Anggota</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {openExpense && <DetailModal expense={openExpense} members={members} onClose={() => setOpen(null)} />}

      <div style={styles.footer}>Data tersimpan di Supabase — semua teman yang buka URL yang sama melihat data yang sama secara real-time.</div>
    </div>
  );
}
...
const styles = {
  ...
  expenseTap: { display: "flex", gap: 12, alignItems: "center", background: "#fff", padding: "14px 14px 14px 16px", borderRadius: 16, marginTop: 10, boxShadow: "0 2px 8px #00000008", cursor: "pointer", transition: "transform .1s ease", position: "relative" },
  expDesc:    { fontWeight: 700, fontSize: 16, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
